...
/**
 * @brief Construct a new Peak Handler:: Peak Handler object
 * 
 * @param samplingFrequency {float} Sampling frequency of the signal
 * @param nr_freq_peaks {int} Number of peaks to be found
 * @param signal_sample_nr {int} Number of samples of the signal
 * @param peaks_value {float*} Pointer to the array of peaks values
 * @param peaks_amps {float*} Pointer to the array of peaks amplitudes
 * @param signal_amplitude {float*} Pointer to the array of signal amplitudes (mg)
 * @param signal_abcissa {float*} Pointer to the array of signal abcissa (frequency)
 */
...
void PeakHandler::get_peaks_zscore(){
	...
	peakDetection.begin(int(this->_signal_sample_nr/4), 5, 0.8);  // sets the lag, threshold and influence

    uint16_t zscore_peak_vector[this->_signal_sample_nr]={};
    ...
    uint16_t zscore_peak_counter = 0;
