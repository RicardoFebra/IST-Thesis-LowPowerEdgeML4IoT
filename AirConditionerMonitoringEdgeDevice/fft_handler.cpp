...
/**
 * @brief Construct a new fft handler::fft handler object
 * 
 * @param raw_data_sample_nr {int} Number of samples in the raw data array
 * @param windows_nr {int} Number of windows to be used in the FFT
 * @param raw_data {int*} Pointer to the raw data array
 * @param fft_amps {float*} Pointer to the FFT amplitudes array
 */
...
/**
 * @brief Window the FFT_handler->_raw_data array with the selected window type
 * 
 * @see Class#FFT_handler(int raw_data_sample_nr, int windows_nr, int *raw_data,float *fft_amps)
 * @param window {uint8_t} Byte to define the window type
 */
...
/**
 * @brief Compute the FFT and store the result in FFT_handler->_fft_amps
 * 
 * @param convert_units {byte} Byte to define if the FFT amplitudes should be converted to the basic units
 * @param normalise_with_window_sample_nr {byte} Byte to define if the FFT amplitudes should be normalised with the number of samples in the window
 * @param normalise_for_half_window {byte} Byte to define if the FFT amplitudes should be normalised for half the window
 */
