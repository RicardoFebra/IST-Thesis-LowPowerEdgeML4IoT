...
void aproxFFT_func(int in[],int N,float *fft_amps){ 
  int a=N,c1,f,o=11,x,data_max,data_min=0;
  ...
  for(int i=0;i<14;i++)                 //calculating   the levels
    {if(Pow2[i]<=a){o=i;}}

  int out_r[a]={};   //real part of transform
  int out_im[a]={};  //imaginary part of transform
  ...
  // example: if data_mag = 1004, avg = 514, scale will be 1 to mantain values in range of -512 to 512 with the average of 0
  // by removing the mean from the data, the intensity of all frequencies besides the DC component will be the same and the DC component will be 0
  // this is okay since DC component is not important this work application
  ...
  for(int i=0;i<o;i++){                                    //fft
    ...
    for(int j=0;j<i10;j++){
      ...
      for(int k=0;k<i11;k++){
        temp4=i10+n1;
        if(c==0)   {tr=out_r[temp4];ti=out_im[temp4];}
        else if(c==256) {tr= -out_im[temp4];ti=out_r[temp4];}
        else if(c==512) {tr=-out_r[temp4];ti=-out_im[temp4];}
        else if(c==768) {tr=out_im[temp4];ti=-out_r[temp4];}
        else if(c==1024){tr=out_r[temp4];ti=out_im[temp4];}
        else{
  ...
  int aux_var = 0;
    for(int i=0;i<Pow2[o-1];i++){               // getting amplitude from compex   number from Pow2[o]/2 to number of sample
      aux_var=fastRSS(out_r[i],out_im[i]);
      fft_amps[i]=(static_cast<float>(aux_var));
      // Approx RSS function used to calculated magnitude quickly  
    }

  /*for(int i=0;i<a/2;i++)
  {
  Serial.print(fft_amps[i]);Serial.print("\	");                     // un comment to print RAW o/p
  Serial.print(out_r[i]);Serial.print("\	");                     // un comment to print RAW o/p    
  Serial.print(out_im[i]);Serial.print("i");Serial.print("\	"); 
  ...
  if(scale<128){
    ...
    for(int i=0;i<Pow2[o-1];i++){
      fft_amps[i]=fft_amps[i]*Pow2[scale];
    }
    scale=0;  
  }   
}
